...
// ✅ Nueva función por documento
export async function getComparendosByDocumento(documento: string): Promise<Comparendo[]> {
  const response = await apiClient.get<ApiResponse<Comparendo[]>>(
    `${API_URLS.comparendos}/comparendos/documento/${documento}`,
  )
  return response.data.data.map(c => ({ ...c, comparendo_id: (c as any).id }))
}
...
export async function createComparendo(data: CreateComparendoPayload): Promise<Comparendo> {
