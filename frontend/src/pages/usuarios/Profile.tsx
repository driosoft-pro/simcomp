...
function Perfil() {
  const { user } = useAuth()
  const { data: usuario, isLoading: loadingUser } = useUsuario(user?.id || '')
  ...
  const canEditEmail = true // All can edit own email
  const canEditAddress = true // All can edit own address
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end pt-4">
          <button
            type="submit"
            disabled={updateUsuarioMutation.isPending || updatePersonaMutation.isPending}
