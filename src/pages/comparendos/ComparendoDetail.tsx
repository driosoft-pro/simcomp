...
function ComparendoDetail() {
  ...
  const handleAnular = async () => {
    if (!data) return
    try {
      await anularMutation.mutateAsync(data.comparendo_id)
      setShowAnularConfirm(false)
      addToast('Comparendo anulado correctamente', 'success')
    } catch (err) {
      addToast('Error al anular el comparendo', 'error')
    }
  }
  ...
  const handleRevertir = async () => {
    if (!data) return
    try {
      await revertirMutation.mutateAsync(data.comparendo_id)
      setShowRevertirConfirm(false)
      addToast('Estado revertido correctamente', 'info')
    } catch (err) {
      addToast('Error al revertir el estado', 'error')
    }
  }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
          <div className="w-full max-w-sm rounded-lg bg-white shadow-2xl overflow-hidden flex flex-col font-mono text-xs text-slate-800 border-t-8 border-emerald-600">
            <div className="p-6 flex flex-col items-center border-b border-dashed border-slate-300">
              ...
              <p className="text-center mt-2 font-bold bg-slate-100 px-3 py-1 rounded inline-block">
                PAGADO
              </p>
            </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex justify-between border-b border-dashed border-slate-200 pb-2">
                <span className="text-slate-500">Fecha Fecha:</span>
